...
headers =  ['Cluster_ID', "Center_Point", "X", "Y"]
gonzalez_report = pd.DataFrame([],columns=headers)
for key, value in data_points2.items():
    center = array_ofj[key-1]
    value =  list(value)
    rep = pd.DataFrame([["Cluster_" + str(int(center)), centers.get(center), value[0], value[1] ]], columns=headers)
    gonzalez_report = gonzalez_report.append(rep)

sns.scatterplot(data=gonzalez_report, x='X', y='Y', hue='Cluster_ID')
